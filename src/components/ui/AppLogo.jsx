...
const LogoLink = styled(Link)`
  ...
  background: ${p => p.$dark
    ? `linear-gradient(100deg, ${tokens.sky200} 0%, ${tokens.sky500} 55%, ${tokens.mint500} 45%, ${tokens.sky200} 100%)`
    : `linear-gradient(100deg, ${tokens.sky700} 0%, ${tokens.sky500} 60%, ${tokens.mint500} 40%, ${tokens.sky700} 100%)`
  };
...
const Star = styled.svg`
  width: ${p => p.$size || '1.6rem'};
  height: ${p => p.$size || '1.6rem'};
  flex-shrink: 0;
  animation: ${spinWithDelay} 5s linear infinite;
`;

export default function AppLogo({ to = '/', fontSize, iconSize, dark = false }) {
  return (
    <LogoLink to={to} $size={fontSize} $dark={dark}>
      <Star viewBox="0 0 32 32" xmlns="http://www.w3.org/2000/svg" $size={iconSize}>
