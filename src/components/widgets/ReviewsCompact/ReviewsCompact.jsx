...
export default function ReviewsCompact({ settings = {}, reviews = [], isMobile = false }) {
  const s = settings;
  ...
  const cols = Number(isMobile ? (s.mobileColumns || 1) : (s.desktopColumns || 2)) || 1;
  ...
  return (
    ...
    >
      ...
      {processed.length === 0 ? (
        ...
        <Grid $cols={cols} $mobileCols={mobileCols} $gap={s.gridGap}>
          {visible.map((review) => {
